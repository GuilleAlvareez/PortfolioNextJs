...
export default function Hero() {
  ...
  return (
    <section
      id="hero"
      className="relative flex flex-col items-center justify-center text-center h-full min-h-screen pt-16 overflow-hidden px-4 md:px-0"
    ...
    >
      ...
      />
      <div
        className="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-gray-900 via-gray-900/50 to-transparent z-20 pointer-events-none"
        aria-hidden="true"
      ></div>
      <div
        className="absolute inset-0 bg-[url('data:image/svg+xml;base64,...')] opacity-20"
        aria-hidden="true"
      ></div>
      ...
      */}
      <div
        className={`relative z-10 space-y-8 transition-all duration-1000 pb-24 ${
          isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
        }`}
      >
        {/* Badge */}
        <div
          className="inline-flex items-center gap-2 bg-gradient-to-r from-cyan-500/10 to-teal-500/10 border border-cyan-400/20 rounded-full px-4 py-2 text-sm text-cyan-300 backdrop-blur-sm"
          role="status"
          aria-live="polite"
        >
          <div
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-center">
            <div
              className="h-1 w-32 bg-gradient-to-r from-cyan-400 to-teal-400 rounded-full"
              aria-hidden="true"
            ></div>
          </div>
        </div>
        ...
        <div className="space-y-6 max-w-3xl mx-auto">
          <p className="text-xl md:text-2xl lg:text-3xl text-gray-200 font-medium leading-relaxed">
            {language === "es" ? (
              <>
                Desarrollador{" "}
                <span className="text-cyan-400 font-bold">Fullstack</span>{" "}
                especializado en{" "}
                <span className="text-teal-400 font-bold">React</span> y
              ...
              <>
                ...
                <span className="text-teal-400 font-bold">React</span> and
          ...
          <p className="text-lg md:text-xl text-gray-400 leading-relaxed px-4">
            {language === "es" ? (
              <>
                ...
                <span className="text-cyan-300">modernas y fluidas</span>, con
                interés en arquitecturas innovadoras como{" "}
              ...
              <>
                ...
                <span className="text-cyan-300">modern and fluid</span> web
                experiences, with interest in innovative architectures like{" "}
                <span className="text-teal-300 font-semibold">Next.js</span> to
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <a
            href="/CV_FullStack_Guillermo_Alvarez (1).pdf"
            ...
            aria-label={t("aria_download_cv")}
          >
            <span className="relative z-10">{t("hero_download_cv")}</span>
            <div
              className="absolute inset-0 bg-gradient-to-r from-cyan-600 to-teal-600 rounded-2xl blur opacity-0 group-hover:opacity-50 transition-all duration-300"
              aria-hidden="true"
            ></div>
          </a>
          <a
            ...
            aria-label={t("aria_view_projects")}
          >
            <span className="relative z-10">{t("hero_view_projects")}</span>
          </a>
        </div>
        ...
        <nav
          ...
          aria-label={t("aria_social_links")}
        >
          <a
